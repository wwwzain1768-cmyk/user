...
function buildRealSubscriptionDates(baseIso) {
    let start = baseIso ? new Date(baseIso) : new Date();
    let end = new Date(start.getTime() + getDurationMs());
    return {
        startDateTime: start.toISOString(),
        endDateTime: end.toISOString(),
        startDate: getDatePartFromIso(start.toISOString()),
        endDate: getDatePartFromIso(end.toISOString())
    };
}
...
function normalizeCustomerDates(customer) {
    ...
    if (!customer.startDateTime || !customer.endDateTime) {
        let baseIso = customer.startDateTime || getNowIso();
        if (!customer.startDateTime && customer.startDate) {
            let startGuess = new Date(customer.startDate);
            if (!isNaN(startGuess.getTime())) {
                let now = new Date();
                startGuess.setHours(now.getHours(), now.getMinutes(), now.getSeconds(), 0);
                baseIso = startGuess.toISOString();
            }
        }
        let built = buildRealSubscriptionDates(baseIso);
...
window.addCustomer = async function() {
    ...
    if (editCustomerId === null) {
        let realDates = buildRealSubscriptionDates();
        let newCustomer = {
            id: Date.now(),
        ...
        for (let i = 0; i < allCustomersData.length; i++) {
            if (allCustomersData[i].id === editCustomerId) {
                let realDates = buildRealSubscriptionDates(allCustomersData[i].startDateTime || getNowIso());
